...
public record FwaystoneLandmark(BlockPos pos, String hash, int fullColor, Text name) implements Landmark<FwaystoneLandmark> {
	public static LandmarkType<FwaystoneLandmark> TYPE = new SimpleLandmarkType<>(Surveystones.id("fwaystone"), pos -> RecordCodecBuilder.create(instance -> instance.group(
		...
		TextCodecs.CODEC.fieldOf("name").orElse(null).forGetter(FwaystoneLandmark::name)
